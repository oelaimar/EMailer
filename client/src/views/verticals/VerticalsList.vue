...
import { getVerticals, deleteVertical, bulkActionVerticals } from '../../api/verticals';
...
const actions = [
  { label: 'Edit', class: 'bg-blue-100 text-blue-700 hover:bg-blue-200', handler: (row) => router.push(`/verticals/${row.id}/edit`) },
  { label: 'Delete', class: 'bg-red-100 text-red-700 hover:bg-red-200', handler: (row) => {
    confirmMessage.value = `Delete Vertical "${row.name}"?`;
    confirmAction.value = () => deleteVertical(row.id).then(() => tableRef.value?.loadData());
...
const handleGroupAction = async ({ action, ids }) => {
  confirmMessage.value = `${action} ${ids.length} Vertical(s)?`;
  confirmAction.value = () => bulkActionVerticals(action, ids).then(() => tableRef.value?.loadData());
  confirmDialog.value = true;
};
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Verticals</h1>
      <router-link to="/verticals/add" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
        + Add New Vertical
      </router-link>
    </div>
    ...
    <DataTable
      ref="tableRef"
      :columns="columns"
      :fetch-data="async (params) => (await getVerticals(params)).data"
      :selectable="true"
      :actions="actions"
