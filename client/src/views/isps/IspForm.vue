...
import { getIsp, createIsp, updateIsp } from '../../api/isps';
...
onMounted(async () => {
  if (isEdit.value) {
    try {
      const { data } = await getIsp(route.params.id);
      form.value = { ...form.value, ...data };
    } catch (e) {
      error.value = 'Failed to load ISP.';
    }
  }
});
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await updateIsp(route.params.id, form.value);
    } else {
      await createIsp(form.value);
    }
    router.push('/isps');
  } catch (e) {
    error.value = e.response?.data?.error || 'Failed to save ISP.';
  } finally {
    loading.value = false;
  }
};
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">{{ isEdit ? 'Edit' : 'Add New' }} ISP</h1>
      <router-link to="/isps" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition-colors">
        Back to List
      </router-link>
    </div>
