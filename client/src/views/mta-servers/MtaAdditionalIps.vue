...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Configure Additional IPs</h1>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <select v-model="selectedServerId" @change="handleServerChange" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
            <option :value="null">Select a server...</option>
            <option v-for="s in servers" :key="s.id" :value="s.id">{{ s.name }} ({{ s.mainIp }}) — {{ s.os }}</option>
          </select>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">OS Format</label>
      ...
      <div class="mt-4">
        <label class="block text-sm font-medium text-gray-700 mb-1">IP Lines</label>
        <textarea v-model="lines" rows="8" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-blue-500 outline-none" :placeholder="selectedOs === 'ubuntu' || selectedOs === 'debian' ? '10.0.0.5\n10.0.0.6' : '10.0.0.5|255.255.255.0|10.0.0.1'"></textarea>
      </div>

      <div class="flex justify-end mt-4">
        <button @click="handleConfigure" :disabled="loading || !selectedServerId" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-sm font-medium rounded-lg transition-colors">
