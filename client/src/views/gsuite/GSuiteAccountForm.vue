...
import { getGsuiteAccount, createGsuiteAccount, updateGsuiteAccount } from '../../api/gsuiteAccounts';
...
onMounted(async () => {
  if (isEdit.value) {
    loading.value = true;
    try {
      const { data } = await getGsuiteAccount(route.params.id);
      form.value = { ...form.value, ...data };
    } catch (e) {
      error.value = 'Failed to load G Suite account.';
    } finally {
      loading.value = false;
    }
  }
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await updateGsuiteAccount(route.params.id, form.value);
    } else {
      await createGsuiteAccount(form.value);
    }
    router.push('/gsuite-accounts');
  } catch (e) {
...
<template>
  <div class="max-w-3xl">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">{{ isEdit ? 'Edit G Suite Account' : 'Add New G Suite Account' }}</h1>
      <router-link to="/gsuite-accounts" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition-colors">Back to List</router-link>
    </div>
