...
import { getDataProviders, deleteDataProvider, bulkActionDataProviders } from '../../api/dataProviders';
...
const actions = [
  { label: 'Edit', class: 'bg-blue-100 text-blue-700 hover:bg-blue-200', handler: (row) => router.push(`/data-providers/${row.id}/edit`) },
  { label: 'Delete', class: 'bg-red-100 text-red-700 hover:bg-red-200', handler: (row) => {
    confirmMessage.value = `Delete data provider "${row.name}"?`;
    confirmAction.value = () => deleteDataProvider(row.id).then(() => tableRef.value?.loadData());
...
const handleGroupAction = async ({ action, ids }) => {
  confirmMessage.value = `${action} ${ids.length} data provider(s)?`;
  confirmAction.value = () => bulkActionDataProviders(action, ids).then(() => tableRef.value?.loadData());
  confirmDialog.value = true;
};
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Data Providers</h1>
      <router-link to="/data-providers/add" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
        + Add New Provider
      </router-link>
    </div>
    ...
    <DataTable
      ref="tableRef"
      :columns="columns"
      :fetch-data="async (params) => (await getDataProviders(params)).data"
      :selectable="true"
      :actions="actions"
