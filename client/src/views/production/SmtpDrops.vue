...
import { getSmtpDrops, executeProcessAction } from '../../api/production';
...
const actions = [
  { label: 'Pause', class: 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200', handler: (row) => {
    confirmMessage.value = `Pause SMTP drop #${row.id}?`;
  ...
  { label: 'Stop', class: 'bg-red-100 text-red-700 hover:bg-red-200', handler: (row) => {
    confirmMessage.value = `Stop SMTP drop #${row.id}?`;
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">SMTP Drops</h1>
    ...
    <DataTable
      ref="tableRef"
      :columns="columns"
      :fetch-data="async (params) => (await getSmtpDrops(params)).data"
      :selectable="true"
      :actions="actions"
    >
