...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <PageHeader title="Launch Campaign — {{ productionName }}" />
      <router-link :to="`/production/${productionId}/processes`" class="px-4 py-2 border border-border bg-surface text-fg hover:bg-surface-alt text-sm font-medium rounded-lg transition-colors">Back to Processes</router-link>
    </div>
    ...
    <div class="bg-surface rounded-xl border border-border p-4 mb-6">
      <div class="flex items-center">
        <div v-for="(name, idx) in stepNames" :key="idx" class="flex items-center">
          <div :class="['w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold', step > idx + 1 ? 'bg-success text-white' : step === idx + 1 ? 'bg-primary text-white' : 'bg-surface-alt text-muted']">
            {{ step > idx + 1 ? '&#10003;' : idx + 1 }}
          </div>
          <span :class="['ml-2 text-sm', step === idx + 1 ? 'font-semibold text-fg' : 'text-muted']">{{ name }}</span>
          <div v-if="idx < stepNames.length - 1" class="w-12 h-px bg-border mx-3"></div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-surface rounded-xl border border-border p-6 mb-6">
      <div v-if="step === 1">
        <h2 class="text-lg font-semibold text-fg mb-4">Campaign Identity</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-fg-secondary mb-1">Process Name *</label>
            <input v-model="form.processName" type="text" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Summer Blast 2026" />
          </div>
          <div>
      ...
      <div v-if="step === 2">
        <h2 class="text-lg font-semibold text-fg mb-4">Email Content</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="md:col-span-2">
            <div class="flex items-center justify-between mb-1">
              <label class="block text-sm font-medium text-fg-secondary">Subject Line</label>
              <div class="flex gap-1">
                <button @click="insertMergeTag('{{firstname}}')" class="px-2 py-0.5 text-xs bg-surface-alt text-muted rounded hover:bg-border">First Name</button>
                <button @click="insertMergeTag('{{lastname}}')" class="px-2 py-0.5 text-xs bg-surface-alt text-muted rounded hover:bg-border">Last Name</button>
                <button @click="insertMergeTag('{{email}}')" class="px-2 py-0.5 text-xs bg-surface-alt text-muted rounded hover:bg-border">Email</button>
              </div>
            </div>
            <input v-model="form.subject" type="text" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Subject line with {{merge_tags}}" />
          </div>
          <div>
          ...
          <div>
            <label class="block text-sm font-medium text-fg-secondary mb-1">Preview Text</label>
            <input v-model="form.previewText" type="text" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div>
            <label class="block text-sm font-medium text-fg-secondary mb-1">Preheader</label>
            <input v-model="form.preheaderText" type="text" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div>
            <label class="block text-sm font-medium text-fg-secondary mb-1">Link Type</label>
            <select v-model="form.linkType" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
          ...
          <div v-if="form.linkType === 'custom'">
            <label class="block text-sm font-medium text-fg-secondary mb-1">Tracking Domain</label>
            <input v-model="form.trackingDomain" type="text" placeholder="track.yourdomain.com" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div class="md:col-span-2 flex items-center gap-6">
            <label class="flex items-center gap-2 text-sm">
              <input v-model="form.unsubscribeLink" type="checkbox" class="rounded" />
              <span class="text-fg-secondary">Include Unsubscribe Link</span>
            </label>
          </div>
          <div class="md:col-span-2">
            <label class="block text-sm font-medium text-fg-secondary mb-1">Physical Address (CAN-SPAM)</label>
            <input v-model="form.physicalAddress" type="text" placeholder="123 Main St, City, State 12345" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
        </div>
      </div>

      <div v-if="step === 3">
        <h2 class="text-lg font-semibold text-fg mb-4">Targeting & Routing</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
          ...
          <div>
            <label class="block text-sm font-medium text-fg-secondary mb-1">Sender Score Threshold</label>
      ...
      <div v-if="step === 4">
        <h2 class="text-lg font-semibold text-fg mb-4">Advanced Settings</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input v-model.number="form.speed" type="number" min="1" max="100" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div>
      ...
      <div v-if="step === 5">
        <h2 class="text-lg font-semibold text-fg mb-4">Launch Campaign</h2>
        <div class="bg-surface-alt rounded-lg p-4 mb-4 space-y-2 text-sm">
          <div class="grid grid-cols-2 gap-2">
            <div><span class="text-muted">Process:</span> <span class="font-medium text-fg">{{ form.processName || '-' }}</span></div>
            <div><span class="text-muted">Subject:</span> <span class="font-medium text-fg">{{ form.subject || '-' }}</span></div>
            <div><span class="text-muted">From:</span> <span class="font-medium text-fg">{{ form.fromName }} &lt;{{ form.fromEmail }}&gt;</span></div>
            <div><span class="text-muted">Offer:</span> <span class="font-medium text-fg">{{ offers.find(o => o.id == form.offerId)?.name || '-' }}</span></div>
            <div><span class="text-muted">Data List:</span> <span class="font-medium text-fg">{{ dataLists.find(d => d.id == form.dataListId)?.name || '-' }}</span></div>
            <div><span class="text-muted">SMTP Group:</span> <span class="font-medium text-fg">{{ smtpGroups.find(g => g.id == form.smtpGroupId)?.name || '-' }}</span></div>
            <div><span class="text-muted">MTA:</span> <span class="font-medium text-fg">{{ mtaServers.find(m => m.id == form.mtaServerId)?.name || '-' }}</span></div>
            <div><span class="text-muted">Speed:</span> <span class="font-medium text-fg">{{ form.speed }}%</span></div>
            <div><span class="text-muted">Batch:</span> <span class="font-medium text-fg">{{ form.batchSize }} / {{ form.batchDelay }}s delay</span></div>
            <div><span class="text-muted">Rotation:</span> <span class="font-medium text-fg">{{ form.rotationType }}</span></div>
            <div><span class="text-muted">Encoding:</span> <span class="font-medium text-fg">{{ form.contentEncoding }}</span></div>
            <div><span class="text-muted">Link Type:</span> <span class="font-medium text-fg">{{ form.linkType }}</span></div>
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="flex items-center gap-2 text-sm mb-2">
              ...
              <span class="text-fg-secondary font-medium">Test Mode (send test emails only)</span>
            </label>
            <textarea v-if="form.testMode" v-model="form.testEmails" rows="3" placeholder="test@email.com, other@email.com" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"></textarea>
          </div>
    ...
    <div class="flex justify-between">
      <button v-if="step > 1" @click="prevStep" class="px-6 py-2 bg-surface-alt hover:bg-border text-fg-secondary text-sm font-medium rounded-lg transition-colors">Previous</button>
      <div v-else></div>
      <button v-if="step < totalSteps" @click="nextStep" :disabled="!canProceed" class="px-6 py-2 bg-primary hover:bg-primary-hover disabled:bg-blue-300 text-white text-sm font-medium rounded-lg transition-colors">Next</button>
    </div>
  </div>
</template>