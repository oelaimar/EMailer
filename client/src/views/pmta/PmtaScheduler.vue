...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <PageHeader title="PMTA Scheduler" />
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-surface rounded-xl border border-border p-6 w-full max-w-lg mx-4">
        <h3 class="text-lg font-semibold text-fg mb-4">Add Scheduled Command</h3>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-fg-secondary mb-1">Delay</label>
              <input v-model="form.delay" type="number" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:border-primary outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-fg-secondary mb-1">Unit</label>
              <select v-model="form.delayUnit" class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:border-primary outline-none">
