...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">{{ isEdit ? 'Edit' : 'Add New' }} Team</h1>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="mb-6">
          <label class="block text-sm font-medium text-gray-700 mb-2">ISPs</label>
          <div class="flex gap-2">
            <button v-for="isp in ispOptions" :key="isp" type="button" @click="toggleIsp(isp)"
              :class="['px-4 py-2 text-sm rounded-lg border transition-colors', form.isps.includes(isp) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50']">
              {{ isp }}
            </button>
          </div>
        </div>
