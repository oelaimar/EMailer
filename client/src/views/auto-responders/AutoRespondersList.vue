...
import { getAutoResponders, deleteAutoResponder, bulkActionAutoResponders } from '../../api/autoResponders';
...
const columns = [
  ...
  { key: 'delay', label: 'Delay (s)' },
  { key: 'status', label: 'Status' },
  { key: 'createdAt', label: 'Created' },
];

const actions = [
  { label: 'Edit', class: 'bg-blue-100 text-blue-700 hover:bg-blue-200', handler: (row) => router.push(`/auto-responders/${row.id}/edit`) },
  { label: 'Delete', class: 'bg-red-100 text-red-700 hover:bg-red-200', handler: (row) => {
    confirmMessage.value = `Delete auto responder "${row.name}"?`;
    confirmAction.value = () => deleteAutoResponder(row.id).then(() => tableRef.value?.loadData());
...
const handleGroupAction = async ({ action, ids }) => {
  confirmMessage.value = `${action} ${ids.length} auto responder(s)?`;
  confirmAction.value = () => bulkActionAutoResponders(action, ids).then(() => tableRef.value?.loadData());
  confirmDialog.value = true;
};
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Auto Responders</h1>
      <router-link to="/auto-responders/add" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
    ...
    <DataTable
      ref="tableRef"
      :columns="columns"
      :fetch-data="async (params) => (await getAutoResponders(params)).data"
      :selectable="true"
      :actions="actions"
