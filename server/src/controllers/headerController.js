...
exports.create = async (req, res, next) => {
  try {
    ...
    logAction(req.user?.email, 'Header', 'create', item.id, item.name, req.user?.id).catch(() => {});
...
exports.update = async (req, res, next) => {
  try {
    ...
    logAction(req.user?.email, 'Header', 'update', item.id, item.name, req.user?.id).catch(() => {});
    res.json(item);
  } catch (error) {
    next(error);
  }
...
exports.remove = async (req, res, next) => {
  try {
    ...
    logAction(req.user?.email, 'Header', 'delete', id, null, req.user?.id).catch(() => {});
