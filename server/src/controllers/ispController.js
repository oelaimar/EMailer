...
exports.list = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.isp.findMany({ where, orderBy, skip, take: limit, select }),
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const item = await prisma.isp.findUnique({ where: { id }, select });
    if (!item) return res.status(404).json({ error: 'ISP not found.' });
    res.json(item);
  } catch (error) {
    next(error);
  }
...
exports.create = async (req, res, next) => {
  try {
    ...
    const item = await prisma.isp.create({
      data: {
        name,
    ...
    logAction(req.user?.email, 'Isp', 'create', item.id, item.name, req.user?.id).catch(() => {});
...
exports.update = async (req, res, next) => {
  try {
    ...
    const item = await prisma.isp.update({
      where: { id },
      data: {
        ...(name !== undefined && { name }),
    ...
    logAction(req.user?.email, 'Isp', 'update', item.id, item.name, req.user?.id).catch(() => {});
    res.json(item);
  } catch (error) {
    next(error);
  }
...
exports.remove = async (req, res, next) => {
  try {
    ...
    logAction(req.user?.email, 'Isp', 'delete', id, null, req.user?.id).catch(() => {});
...
exports.bulkAction = async (req, res, next) => {
  try {
    ...
    switch (action) {
      case 'activate':
        await prisma.isp.updateMany({ where: { id: { in: intIds } }, data: { status: 'Activated' } });
        break;
      case 'inactivate':
        await prisma.isp.updateMany({ where: { id: { in: intIds } }, data: { status: 'Inactivated' } });
        break;
      case 'delete':
        await prisma.isp.deleteMany({ where: { id: { in: intIds } } });
        break;
      default:
...
exports.bulkAdd = async (req, res, next) => {
  try {
    ...
    const created = await prisma.isp.createMany({
      data: nameList.map((name) => ({ name, status: 'Activated', createdBy })),
    });

    res.status(201).json({ message: `${created.count} ISPs created.`, count: created.count });
  } catch (error) {
    next(error);
  }
};
