...
exports.create = async (req, res, next) => {
  try {
    ...
    logAction(req.user?.email, 'GsuiteAccount', 'create', item.id, item.email, req.user?.id).catch(() => {});
...
exports.update = async (req, res, next) => {
  try {
    ...
    logAction(req.user?.email, 'GsuiteAccount', 'update', item.id, item.email, req.user?.id).catch(() => {});
    res.json(item);
  } catch (error) {
    next(error);
  }
...
exports.sendProcessData = async (req, res, next) => {
  try {
    const [accounts, smtpGroups, offers, headers] = await Promise.all([
      prisma.gSuiteAccount.findMany({ where: { status: 'Activated' }, select: { id: true, email: true } }),
